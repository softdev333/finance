...
public class SheetView extends VerticalLayout implements BeforeEnterObserver{
	...
	public SheetView(SheetService sheetService, ExpenseService expenseService, IncomeService incomeService ) throws IOException, GeneralSecurityException
	{
		...
		try {
			...
			//rowNumber by default will start with 0 i.e. 0 rows read, and 1st row will have row headers
	...
	private void convertTransactionAndAddToList(TransactionDto transactionDto, List<Income> incomeList,	List<Expense> expenseList) {
	...
	private TransactionDto convertDataToTransactionDto(List<Object> row, String userId) {
		...
		LocalDate transactionDate = LocalDate.of(Integer.parseInt(dateParts[2]) , Integer.parseInt(dateParts[1]), Integer.parseInt(dateParts[0]));
		transactionDto.setTransactionDate(transactionDate);
		
		String transactionTypeString = (String) row.get(1);
		TransactionType transactionType = transactionTypeString.toLowerCase().equals("income") ? TransactionType.INCOME : TransactionType.EXPENSE;
