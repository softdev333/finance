...
public class IncomeView extends VerticalLayout implements BeforeEnterObserver {
	...
	arrowRightButton.addClickListener(event -> {
	    ...
	    List<Income> updatedIncomeList = incomeService.getAllIncomeByUserId(userId, latestPageable);
	    updateIncomeGrid(updatedIncomeList);
	    VaadinSession.getCurrent().setAttribute(INCOME_PAGE_NUMBER,
		    updatedIncomePageNumber < 0 ? 0 : updatedIncomePageNumber);
	});
	...
	String userId = (String) VaadinSession.getCurrent().getAttribute(USER_ID);
	totalIncomeField
		.setValue(incomeService.getAllIncomeForCurrentMonthForUser(userId, LocalDate.now()).toPlainString());
    }
